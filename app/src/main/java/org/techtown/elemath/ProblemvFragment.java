...
public class ProblemvFragment extends Fragment {

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        ViewGroup rootView = (ViewGroup) inflater.inflate(R.layout.fragment_problemv, container, false);
        ...
        EditText answerEditTextv = rootView.findViewById(R.id.answerEditTextv);
        MathSuccessFragment successFragment = new MathSuccessFragment();

        Button confirmButtonv = rootView.findViewById(R.id.confirmButtonv);
        confirmButtonv.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (!("".equals(answerEditTextv.getText().toString())) && Integer.parseInt(answerEditTextv.getText().toString()) == 126273) {
                    ...
                    if (mathProblemFragment.getSelectGame()) {
                        editor.putBoolean("ProblemV", true);
                        editor.commit();
                    }
                } else {
        ...
        HintvFragment hintvFragment = new HintvFragment();
        Button hintButtonv = rootView.findViewById(R.id.hintButtonv);
        hintButtonv.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                SoundSet.play(SoundSet.buttonSound);
                getParentFragmentManager().beginTransaction().replace(R.id.container, hintvFragment).commit();
            }
        });

        Button backButtonv = rootView.findViewById(R.id.backButtonv);
        backButtonv.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                SoundSet.play(SoundSet.buttonSound);
                if (mathProblemFragment.getSelectGame()) {
