...
public class ProblembFragment extends Fragment {

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        ViewGroup rootView = (ViewGroup) inflater.inflate(R.layout.fragment_problemb, container, false);
        ...
        EditText answerEditTextb = rootView.findViewById(R.id.answerEditTextb);
        MathSuccessFragment successFragment = new MathSuccessFragment();

        Button confirmButtonb = rootView.findViewById(R.id.confirmButtonb);
        confirmButtonb.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (!("".equals(answerEditTextb.getText().toString())) && Integer.parseInt(answerEditTextb.getText().toString()) == 3) {
                    ...
                    if (mathProblemFragment.getSelectGame()) {
                        editor.putBoolean("ProblemB", true);
                        editor.commit();
                    }
                } else {
        ...
        HintbFragment hintbFragment = new HintbFragment();
        Button hintButtonb = rootView.findViewById(R.id.hintButtonb);
        hintButtonb.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                SoundSet.play(SoundSet.buttonSound);
                getParentFragmentManager().beginTransaction().replace(R.id.container, hintbFragment).commit();
            }
        });

        Button backButtonb = rootView.findViewById(R.id.backButtonb);
