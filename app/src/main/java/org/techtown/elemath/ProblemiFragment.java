...
public class ProblemiFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        ViewGroup rootView = (ViewGroup) inflater.inflate(R.layout.fragment_problemi, container, false);
        ...
        EditText answerEditTexti = rootView.findViewById(R.id.answerEditTexti);
        MathSuccessFragment successFragment = new MathSuccessFragment();

        Button confirmButtoni = rootView.findViewById(R.id.confirmButtoni);
        confirmButtoni.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (!("".equals(answerEditTexti.getText().toString())) && Integer.parseInt(answerEditTexti.getText().toString()) == 8) {
                    ...
                    if (mathProblemFragment.getSelectGame()) {
                        editor.putBoolean("ProblemI", true);
                        editor.commit();
                    }
                } else {
        ...
        HintiFragment hintiFragment = new HintiFragment();
        Button hintButtoni = rootView.findViewById(R.id.hintButtoni);
        hintButtoni.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                SoundSet.play(SoundSet.buttonSound);
                getParentFragmentManager().beginTransaction().replace(R.id.container, hintiFragment).commit();
            }
        });

        Button backButtoni = rootView.findViewById(R.id.backButtoni);
