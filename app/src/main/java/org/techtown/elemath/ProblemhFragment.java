...
public class ProblemhFragment extends Fragment {

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        ViewGroup rootView = (ViewGroup) inflater.inflate(R.layout.fragment_problemh, container, false);
        ...
        EditText answerEditTexth = rootView.findViewById(R.id.answerEditTexth);
        MathSuccessFragment successFragment = new MathSuccessFragment();

        Button confirmButtonh = rootView.findViewById(R.id.confirmButtonh);
        confirmButtonh.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (!("".equals(answerEditTexth.getText().toString())) && Integer.parseInt(answerEditTexth.getText().toString()) == 6) {
                    ...
                    if (mathProblemFragment.getSelectGame()) {
                        editor.putBoolean("ProblemH", true);
                        editor.commit();
                    }
                } else {
        ...
        HinthFragment hinthFragment = new HinthFragment();
        Button hintButtonh = rootView.findViewById(R.id.hintButtonh);
        hintButtonh.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                SoundSet.play(SoundSet.buttonSound);
                getParentFragmentManager().beginTransaction().replace(R.id.container, hinthFragment).commit();
            }
        });

        Button backButtonh = rootView.findViewById(R.id.backButtonh);
